...
from core.auth import authenticate_user, create_access_token, decode_access_token
from providers.bocha_api import BochaApiProvider
from providers.doubao_web import ensure_utf8_string
from datetime import datetime, timedelta
from models import (
    MockRequest, MockResponse, StatusResponse,
    LoginRequest, LoginResponse,
    TaskSyncItem, TaskSyncRequest, TaskSyncResponse
)
...
# Pydantic models are now imported from models package
...
@app.post("/client/auth/login", response_model=LoginResponse)
...
@app.post("/client/mytask", response_model=TaskSyncResponse)
async def sync_task_status(
    ...
    try:
        ...
        logger.info(f"用户 {username} (ID: {user_id}) 开始同步 {len(request.tasks)} 个任务")
            ...
            for task_item in request.tasks:
                try:
                    ...
                    cur.execute("""
                        ...
                        (keywords, platforms, query_count, status, 
                         task_type, source, user_id, created_by, created_at, updated_at)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
                        RETURNING id
                    """, (
                        json.dumps([task_item.keywords]),
                        json.dumps([task_item.platforms]),
                        task_item.query_count,
                        task_item.status,
                        task_item.task_type,
                        task_item.source,
                        user_id,
                        task_item.created_by or username,
                    ...
                    synced_tasks.append({
                        "local_task_id": task_item.local_task_id,
                        "server_task_id": server_task_id
                    })
                    
                    logger.info(f"任务已同步：本地ID {task_item.local_task_id} -> 服务端ID {server_task_id}")
                    
                except Exception as e:
                    error_msg = f"任务 {task_item.local_task_id}: {str(e)}"
                    errors.append(error_msg)
                    logger.error(error_msg)
            
            conn.commit()
        
        logger.info(f"同步完成：成功 {len(synced_tasks)} 个，失败 {len(errors)} 个")
